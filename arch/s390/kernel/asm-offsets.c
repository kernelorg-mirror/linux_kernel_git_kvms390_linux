...
int main(void)
{
	...
	BLANK();
	DEFINE(__THREAD_FPU_fpc, offsetof(struct task_struct, thread.fpu.fpc));
	DEFINE(__THREAD_FPU_flags, offsetof(struct task_struct, thread.fpu.flags));
	DEFINE(__THREAD_FPU_regs, offsetof(struct task_struct, thread.fpu.regs));
	BLANK();
