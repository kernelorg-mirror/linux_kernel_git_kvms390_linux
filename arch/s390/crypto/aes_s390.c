		...
		unsigned int nbytes)
{
	...
	struct blkcipher_desc fallback_desc = {
		.tfm = sctx->fallback.blk,
		.info = desc->info,
		.flags = desc->flags
	};

	return crypto_blkcipher_decrypt_iv(&fallback_desc, dst, src, nbytes);
}
		...
		unsigned int nbytes)
{
	...
	return crypto_blkcipher_encrypt_iv(&fallback_desc, dst, src, nbytes);
}
