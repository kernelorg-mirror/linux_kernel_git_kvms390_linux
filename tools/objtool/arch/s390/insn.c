...
struct s390_insn_opcode_offset {
...
static struct s390_insn_opcode_offset opcode_offset[] = OPCODE_OFFSET_INITIALIZER;
...
{
	struct s390_insn_opcode_offset *entry;
	 ...
	 */
	for (i = 0; i < sizeof(opcode_offset)/sizeof(opcode_offset[0]); i++) {
			 ...
			 unsigned int *values)
{
	...
	insn = pinsn;
	ops = formats[insn->format];
	for (i = 0; i < 6 && ops[i] != 0; i++)
		values[i] = extract_operand(code, operands + ops[i]);
}
