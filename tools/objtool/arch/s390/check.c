...
static bool has_valid_stack_frame(struct insn_state *state)
{
	 ...
	 * returns false if r15 still has the original value and
	 * has not been changed. It returns true for all other
 ...
 *
 * For global functions uses_stack is true and in general dasd_end is false.
 ...
 * starting from the state passed in via 'old'.
 */
static int validate_insn(struct objtool_file *file, struct symbol *func,
			 struct instruction *insn, struct insn_state *cur,
			 struct list_head *root)
